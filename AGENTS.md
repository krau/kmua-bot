...
```python
def get_items() -> list[str]: pass
def get_user(id: int) -> UserData | None: pass
```
...
async def start(client: Client, message: Message):
    ...
    await message.reply(i18n.t("bot.msg.private_start", locale=chat_config.lang))
```
...
class UserData(Base):
    ...
    quotes: Mapped[list["Quote"]] = relationship("Quote", back_populates="user", lazy="noload")
```
