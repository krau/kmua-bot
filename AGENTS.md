...
- **Agent subsystem** (`kmua/plugins/agent/`): in-chat AI agent with tools — shell (landrun sandbox, `kmua/services/sandbox.py`), io (AgentFS workspaces `work://`, `kmua://` read-only, `chat://`, `memory://`), tg (Bot API mapping), web (SSRF-guarded via `kmua/common/safe_http.py`).
