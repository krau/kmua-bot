...
async def interact(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        this_mention = this_user.mention_markdown()
    except TypeError:
        this_mention = (
    ...
    if reply_to_message := update.effective_message.reply_to_message:
        ...
        try:
            replied_mention = replied_user.mention_markdown()
        except TypeError:
            replied_mention = f"[{escape_markdown(replied_user.title)}](tg://user?id={replied_user.id})"
    is_backslash = (
        False
