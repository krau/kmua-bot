    ...
    if update.effective_user.id == master_id:
        for blackuser in update.effective_message.entities:
            if blackuser.type == MessageEntityType.MENTION:
                ...
                if helper.black(blackuser_id) == True:
                    await context.bot.send_message(chat_id=update.effective_chat.id, reply_to_message_id=update.effective_message.id, text='已经将加入黑名单')
                else:
                    await context.bot.send_message(chat_id=update.effective_chat.id, reply_to_message_id=update.effective_message.id, text='加入失败,可能是已经在黑名单中')
            else:
                await context.bot.send_message(chat_id=update.effective_chat.id, text='没有要加入黑名单的用户')
    else:
        await context.bot.send_message(chat_id=update.effective_chat.id, text='不行!!不可以!')
