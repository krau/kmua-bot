...
async def 开始(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_sticker(
        ...
        sticker="CAACAgUAAxkBAAM7Y4oxOY0Tkt5D5keXXph7jFE7U7YAAqUCAAJfIulXxC0Bkai8vqwrBA",
    )
...
async def 设置群员权限(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        日志器.info(f"授予{update.effective_user.name} {custom_title}")
        text = f"好,你现在是{custom_title}啦"
        await context.bot.send_message(
            chat_id=chat_id, reply_to_message_id=update.effective_message.id, text=text
        )
...
async def 删除所有模组(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if update.effective_user.id == master_id:
            ...
            日志器.debug("删除:./pics")
            await context.bot.send_message(
                chat_id=update.effective_chat.id, text="已经删除了所有保存的模组数据"
            )
            await context.bot.send_sticker(
                chat_id=update.effective_chat.id,
                sticker="CAACAgUAAxkBAAIFXGOhEyZbeuhLM41Y9BoyZUHAoGdjAAJRBAACV0C5VoqO8DRjKNWPLAQ",
            )
        else:
            await context.bot.send_message(
                chat_id=update.effective_chat.id, text="不行!只有主人才可以!"
            )
    except OSError as e:
        text = f"出错惹~{e}"
        await context.bot.send_message(chat_id=update.effective_chat.id, text=text)
...
async def 入典(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        flag = 小工具.record_msg_id(chat_id=update.effective_chat.id, msg_id=msg_id)
        if flag is True:
...
async def 取消入典(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        flag = 小工具.delete_msg_id(chat_id=update.effective_chat.id, msg_id=msg_id)
        if flag is True:
            await context.bot.send_message(chat_id=update.effective_chat.id, text="已出典")
        else:
            await context.bot.send_message(
                chat_id=update.effective_chat.id, text="不在典里,请回复被置顶的典,而不是被转发的典"
            )
    except Exception as e:
        text = f"不行! {e}"
        await context.bot.send_message(chat_id=update.effective_chat.id, text=text)
...
async def 不要色色(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 小工具.random_with_probability(pr_nosese):
        ...
        await context.bot.send_sticker(
            chat_id=update.effective_chat.id,
            sticker="CAACAgUAAxkBAAM_Y4oxreCJwFtLa1okJMS3Xz7g8UsAAmYCAAImjuhXJN6lY6dZeNUrBA",
        )
...
async def 早安(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 小工具.random_with_probability(pr_ohayo):
        # text = 获取词.get_ohayo()
        stickers = [
            "CAACAgUAAxkBAAPnY4xM4SVJj5T5plvLUc89Lra77IUAAvACAAL30uhX8lw2t4mq6GErBA",
...
async def 晚安(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 小工具.random_with_probability(pr_sleep):
        # text = 获取词.get_wanan()
        stickers = [
            "CAACAgUAAxkBAANEY4oyf4yTWS2IwdQI85PXUX4HQCUAAtkDAAJWFLhWB5Xyu3EaZwcrBA",
...
async def 文爱(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_chat.id != master_id and 偷情监控 is True:
...
async def 发送模组数据(update: Update, context: ContextTypes.DEFAULT_TYPE, data_dict: dict):
    ...
    await context.bot.send_photo(
        chat_id=update.effective_chat.id, photo=photo, caption=text, parse_mode="HTML"
    )
...
def run():
    ...
    开启偷情监控处理器 = CommandHandler("enableaffairnotice", 开启偷情监控)
    关闭偷情监控处理器 = CommandHandler("disableaffairnotice", 关闭偷情监控)
    ...
    输出已经保存的模组处理器 = MessageHandler(filters.Regex("模组列表"), 输出已经保存的模组)
